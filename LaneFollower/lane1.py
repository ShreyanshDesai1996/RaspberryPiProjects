...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if(GPIO.input(rx)):
        print('Obstruction detected by IR sensor')
    
    
    while(dist<7):
        print('Obstruction detected by ultrasonic sensor,stop')
    ...
    m1=cv2.getRotationMatrix2D((160,120),180,1.0)
    image=cv2.warpAffine(image,m1,(320,240))
