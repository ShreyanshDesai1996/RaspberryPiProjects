...
def proceesed_img(original_image):
    ...
    retval,mask_img=cv2.threshold(proceesed_img,0,255,cv2.THRESH_BINARY)
    #Comment below line if error
    #proceesed_img = cv2.threshold(proceesed_img,20,255, THRESH_BINARY)

    proceesed_img = cv2.GaussianBlur(proceesed_img, (5, 5), 0)

    #Comment below line if error
    #proceesed_img=cv2.threshold(proceesed_img,127,256,cv2.THRESH_BINARY)
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    #Comment below line in case of error
    #frame=cv2.threshold(frame,127,256,cv2.THRESH_BINARY)
